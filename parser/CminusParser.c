...
{
       0,   100,   100,   105,   112,   117,   122,   129,   134,   141,
     148,   155,   160,   167,   173,   179,   186,   192,   198,   202,
     206,   210,   214,   218,   222,   228,   244,   248,   255,   261,
     268,   274,   280,   287,   294,   301,   310,   316,   323,   329,
     333,   339,   344,   349,   354,   361,   366,   371,   376,   381,
     386,   391,   398,   403,   408,   415,   420,   425,   432,   446,
     451,   455,   462,   478,   484,   504
};
#endif
...
#line 180 "CminusParser.y"
			...
			SymPutFieldByIndex(symtab,yyvsp[0],SYMTAB_OFFSET_FIELD,gp); 
			yyval = yyvsp[0]; //return index to variable
		}
...
#line 229 "CminusParser.y"
			...
			char *regAddressName = (char*)SymGetFieldByIndex(symtab, yyvsp[-3], SYM_NAME_FIELD); //get name of address register
			char *regValueName = (char*)SymGetFieldByIndex(symtab, yyvsp[-1], SYM_NAME_FIELD); //get name of address register
    		...
    		dlinkAppend(iList,dlinkNodeAlloc(inst)); //add instruction to list
			int reg = SymGetFieldByIndex(symtab, yyvsp[-3], SYMTAB_REGISTER_INDEX_FIELD);
			freeIntegerRegister(reg);
			reg = SymGetFieldByIndex(symtab, yyvsp[-1], SYMTAB_REGISTER_INDEX_FIELD);
			freeIntegerRegister(reg);
		}
...
#line 288 "CminusParser.y"
			...
			readFunc(yyvsp[-2]);
			int reg = (int)SymGetFieldByIndex(symtab, yyvsp[-2], SYMTAB_REGISTER_INDEX_FIELD);
			freeIntegerRegister(reg);
		}
...
#line 345 "CminusParser.y"
			...
			yyval = bitwise(yyvsp[-2], yyvsp[0], 0);
		}
#line 1629 "CminusParser.c"
    break;
...
#line 350 "CminusParser.y"
			...
			yyval = bitwise(yyvsp[-2], yyvsp[0], 1);
		}
...
#line 355 "CminusParser.y"
			...
			yyval = bitwise(yyvsp[0], yyvsp[0], 2);
		}
...
#line 372 "CminusParser.y"
			...
			yyval = comp(yyvsp[-2], yyvsp[0], 1);
		}
#line 1674 "CminusParser.c"
    break;
...
#line 421 "CminusParser.y"
			...
			yyval = advancedMath(yyvsp[-2], yyvsp[0], 0);
		}
...
#line 426 "CminusParser.y"
			...
			yyval = advancedMath(yyvsp[-2], yyvsp[0], 1);
		}
...
#line 433 "CminusParser.y"
			...
			char* regAddressName = (char*)SymGetFieldByIndex(symtab, yyvsp[0], SYM_NAME_FIELD); //get reg name
			...
			dlinkAppend(iList, dlinkNodeAlloc(inst));
			int reg = (int)SymGetFieldByIndex(symtab, yyvsp[0], SYMTAB_REGISTER_INDEX_FIELD);
			freeIntegerRegister(reg);
			yyval = regIndex;
		}
...
#line 463 "CminusParser.y"
			...
			int offset = (int)SymGetFieldByIndex(symtab, yyvsp[0], SYMTAB_OFFSET_FIELD); //get the offset from gp to variable
			...
			free(offsetString);
			yyval = regIndex; //return index of reg
		}
...
#line 485 "CminusParser.y"
			...
			char *varName = (char*)SymGetFieldByIndex(symtab,yyvsp[0],SYM_NAME_FIELD); //get value of string
			...
			dlinkAppend(iList,dlinkNodeAlloc(inst)); //add instruction to list
			free(label);
			yyval = regIndex; //return reg index
		}
#line 1858 "CminusParser.c"
    break;

  case 65: /* Constant: INTCON  */
#line 505 "CminusParser.y"
                { 
			//printf("<Constant> -> <INTCON>\n");
			int regIndex = getFreeIntegerRegisterIndex(symtab); //get register
			char* regName = (char*)SymGetFieldByIndex(symtab,regIndex,SYM_NAME_FIELD); //get name of register
			int size = snprintf(NULL, 0, "%d", yyvsp[0]); //convert int to string
			char *numString = malloc(size+1);
			snprintf(numString, size+1, "%d", yyvsp[0]);
